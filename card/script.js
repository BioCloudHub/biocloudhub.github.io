...
function getVisibleTags() {
  ...
  const roleFilteredData = cardData.filter(
    (item) =>
      selectedRole === "all" ||
      (item.roles && item.roles.includes(selectedRole)),
  );
...
function renderRoleSwitcher() {
  ...
  // 清除旧项（保留 slider）
  const items = roleSwitcherContainer.querySelectorAll(
    ".role-item, .role-more-dropdown",
  );
  items.forEach((item) => item.remove());

  // 定义常用角色的 ID
  const commonRoleIds = ["all", "rd", "quality", "clinical", "production"];
  const commonRoles = roles.filter((r) => commonRoleIds.includes(r.id));
  const otherRoles = roles.filter((r) => !commonRoleIds.includes(r.id));
  ...
  commonRoles.forEach((role) => {
    ...
    roleElement.addEventListener("click", () => {
      ...
      // 如果之前是在更多里选的，移除更多按钮的 active 状态
      const moreDropdown = roleSwitcherContainer.querySelector(
        ".role-more-dropdown",
      );
    ...
    if (selectedRole === role.id) activeElement = roleElement;
  });

  // 渲染“更多”下拉菜单
  const moreDropdown = document.createElement("div");
  moreDropdown.className = "role-more-dropdown relative";
  moreDropdown.id = "roleMoreDropdown";

  const isSelectedInDropdown = otherRoles.some((r) => r.id === selectedRole);
  if (isSelectedInDropdown) moreDropdown.classList.add("active");

  moreDropdown.innerHTML = `
    <div class="role-item more-trigger">
      <span class="role-icon">➕</span>
      <span class="role-name">更多</span>
  ...
  const moreTrigger = moreDropdown.querySelector(".more-trigger");

  otherRoles.forEach((role) => {
    const item = document.createElement("div");
    item.className = `dropdown-item ${selectedRole === role.id ? "active" : ""}`;
    item.innerHTML = `
      <span class="role-icon">${role.icon}</span>
      <span class="role-name">${role.name}</span>
    `;
    ...
    item.addEventListener("click", (e) => {
      e.stopPropagation(); // 防止触发 trigger 的点击
      if (selectedRole === role.id) return;
  ...
  roleSwitcherContainer.appendChild(moreDropdown);

  if (isSelectedInDropdown) {
    activeElement = moreDropdown;
  }

  // 初始化时设置滑块位置
  if (activeElement) {
    setTimeout(() => {
      updateRoleUI(activeElement, false);
      // 重新初始化图标（因为动态生成了 chevron-down）
...
function updateRoleUI(activeElement, animate = true) {
  ...
  roleSwitcherContainer
    .querySelectorAll(".role-item, .role-more-dropdown")
    .forEach((el) => el.classList.remove("active"));
  activeElement.classList.add("active");

  // 计算滑块位置
  // 如果是更多下拉，滑块应该覆盖整个下拉触发器容器
...
function checkAndFillScreen() {
  ...
  const isNearBottom =
    scrollContainer.scrollHeight <= scrollContainer.clientHeight + 200;
...
function initElasticScroll() {
  ...
  const updateGlow = (distance) => {
    ...
    glow.style.transition =
      distance === 0 ? "opacity 0.6s ease, transform 0.6s ease" : "none";
  };
    ...
    (e) => {
      ...
      if (tagFilters) {
        ...
        if (isScrollable) {
          ...
          const isAtTagBottom =
            Math.ceil(tagFilters.scrollTop + tagFilters.clientHeight) >=
            tagFilters.scrollHeight;

          // 只有当在标签区域也触及边界且继续向该方向滚动时，才允许冒泡给父容器处理弹性效果
          if (
            (e.deltaY < 0 && !isAtTagTop) ||
            (e.deltaY > 0 && !isAtTagBottom)
          ) {
            return; // 允许标签区域正常滚动
          }
        }
